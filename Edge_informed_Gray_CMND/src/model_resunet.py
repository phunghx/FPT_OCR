...
class UpBlock(nn.Module):
    ...
    def forward(self, down_input, skip_input):
        ...
        x = self.up_sample(down_input)
        #x = nn.Upsample(size=skip_input.shape[2:])(x)
        #import pdb;pdb.set_trace()
        #skip_input = skip_input[:,:,0:x.shape[2],0:x.shape[3]]
        #if x.shape[2] < skip_input.shape[2]:
        #    #import pdb;pdb.set_trace()
        #    x2 = torch.zeros([x.shape[0],x.shape[1],skip_input.shape[2],skip_input.shape[2]])
        #    x2[:,:,0:x.shape[2],0:x.shape[3]] = x
        #    x = x2.cuda()
        x = torch.cat([x, skip_input], dim=1)
        return self.double_conv(x)
