...
class EdgeMatch():
    def __init__(self, config):
        ...
        if self.config.MODE==2:
             ...
             self.test_dataset =  Dataset('test', sigma=config.SIGMA, scale=config.SCALE, hr_size=config.HR_SIZE, augment=False, lr_flist=config.TEST_FLIST_LR,)
        else:
        ...
        #import pdb;pdb.set_trace()
        #temp_items = self.train_dataset[1000]
        #import pdb;pdb.set_trace()
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if model == 1:
                    # train
                    self.sr_model.eval()
                    hr_images_predict = self.sr_model(lr_images, hr_edges).detach()
                    hr_edges_pred, gen_loss, dis_loss, logs = self.edge_model.process(hr_images_predict, hr_images, lr_edges, hr_edges)
                    # metrics
                    ...
                    try:
                       ...
                       logs.append(('precision',0))
                    try:
                       logs.append(('recall', recall.item()))
                    except:
                       logs.append(('recall',0))
                    # backward
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # sr model / joint model
                ...
                #hr_edges_pred = self.edge_model(lr_images, lr_edges).detach()
                outputs = self.sr_model(lr_images, lr_edges)
    ...
    def sample(self):
        ...
        iteration = self.sr_model.iteration
        hr_edges = lr_edges #self.edge_model(lr_images, lr_edges).detach()
